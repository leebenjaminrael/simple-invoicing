...
- **Documents** — invoices, quotes and **credit notes**: create, send, mark paid, duplicate, convert quote → invoice, export PDF, compose email. Overdue invoices get a one-click **payment reminder** email; quotes track accepted/declined and expire automatically. Set an invoice to **repeat monthly** and you'll be offered a fresh draft each month. Document numbering (prefix + padding, e.g. `INV-2026-001`) is customisable per business, with a separate sequence for credit notes.
  - **Part invoicing** — bill a deposit (25/50/75%) then click **Invoice balance** to raise the linked balance invoice; the app keeps them connected so profit and VAT never double-count, and the dashboard shows how much is *still to invoice* on active jobs.
  - **Credit notes** — a full document type with its own numbering. Create one against an invoice (or convert an invoice into one); issued credit notes reduce your revenue, output VAT and the amount that invoice shows as owed.
- **Clients** — your client list with outstanding balances and a default hourly rate.
- **Projects** *(optional)* — plan a job before you quote it: build a budget from simple lines (your time + a cost or two) up to full film-style budgets with sections, day rates × units, crew rates, and a production fee. See planned billables vs costs vs margin live, toggle sections to price option combinations, then **create the client quote straight from the budget**. Linked invoices, expenses and tracked time roll up into actual profitability per project.
